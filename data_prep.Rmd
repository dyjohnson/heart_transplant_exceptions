...
# rec_exception = 1 when a candidate's status matches their approved exception status and the date is between the exception's effective and expiration dates with a three-day buffer for the effective date
# full_list <- full_list %>%
#   mutate(
...
full_list <- full_list  %>%
  left_join(
    listing_ctrs %>%
      select(PX_ID, CAN_LISTING_CTR_ID, can_listing_ctr_count),
    by = c("PX_ID", "CAN_LISTING_CTR_ID")
  )

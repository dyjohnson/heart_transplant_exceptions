...
full_list_surv <- full_list %>%
  ...
  summarise(
    ...
    rec_exception_init_ever = last(rec_exception_init_ever),
...
t <- survfit(Surv(survival_time, death) ~ rec_exception_init, data = full_list_surv)

x <- survfit(Surv(survival_time, death) ~ rec_exception_init_ever, data = full_list_surv)
...
survplot_ex2 <-
  ggsurvplot(
    fit = survfit(Surv(survival_time, death) ~ rec_exception_init_ever, data = full_list_surv),
    pval = TRUE,
    pval.coord = c(0,0.73),
    conf.int = FALSE,
    risk.table = TRUE,
    ...
    break.time.by = 30,
    ylab = "Survival Probability",
    legend.labs = c("Exception after listing", "No exception"), #"Exception at listing", 
    censor = TRUE,
    risk.table.y.text.col = T,
    risk.table.y.text = FALSE,
    tables.height = 0.31
...
``` {r}
full_list %>% group_by(PX_ID) %>% summarise(PX_ID = last(PX_ID), rec_exception_init_ever = last(rec_exception_init_ever), removal_cause = last(removal_cause)) %>% ungroup() %>% group_by(rec_exception_init_ever) %>% count(removal_cause)
